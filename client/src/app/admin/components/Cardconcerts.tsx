...
export default function Cardconcerts({ concert, onDelete }: CardProps) {
  const title = concert.name ?? "Untitled Concert";
  const description = concert.description ?? "";
  const seats = concert.available_seats ?? concert.seat ?? 0;
  ...
  return (
    ...
    >
      <div className="flex-1">
        ...
        <p className="text-[12px] sm:text-[14px] md:text-[18px] lg:text-[24px] text-gray-600">
          {description}
        </p>
      </div>
      ...
      <div className="flex flex-row items-center gap-4 justify-between">
        <div className="flex items-center gap-2 text-gray-600">
          ...
          />
          <span className="text-[12px] sm:text-[14px] md:text-[18px] lg:text-[24px]">
            {seats}
          </span>
        </div>
